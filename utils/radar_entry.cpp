...
gui::radar_entry::radar_entry(std::shared_ptr<class logger_window> factory, std::string name, double max_r,
                              size_t segments)
    : ilogger_entry(std::move(factory), std::move(name)), max_r_{max_r}, segments_{segments} {}
...
void gui::radar_entry::draw_impl() {
  ...
  for (auto i = 1; i <= segments_; ++i) { dlist->AddCircle(center, dR * i, color, 100, 1.0f); }

  auto convert = [this](double &r, double &phi) {
    if (r >= max_r_) r = std::abs(r) / r * max_r_;
  ...
  for (auto &point : tracked_.main()) {
    ...
    ImVec2 target{center.x + mp.x, center.y + mp.y};
    if (!point.lock_target)
      dlist->AddRect({target.x - 5, target.y - 5}, {target.x + 5, target.y + 5}, ImColor{0, 255, 0}, 2);
    else {
      auto const tsize_x = 7, tsize_y = 5;
      dlist->AddLine({target.x, target.y + tsize_y}, {target.x + tsize_x, target.y - tsize_y}, ImColor{255, 0, 0}, 1);
      dlist->AddLine({target.x + tsize_x, target.y - tsize_y}, {target.x - tsize_x, target.y - tsize_y},
                     ImColor{255, 0, 0}, 1);
      dlist->AddLine({target.x - tsize_x, target.y - tsize_y}, {target.x, target.y + tsize_y}, ImColor{255, 0, 0}, 1);
    }

    for (std::weakly_incrementable auto i : std::views::iota(1ul, point.trajectory.size())) {
      auto p1 = scale_point(ImVec2{static_cast<float>(point.trajectory[i - 1].first),
                                   static_cast<float>(point.trajectory[i - 1].second)});
      auto p2 = scale_point(
          ImVec2{static_cast<float>(point.trajectory[i].first), static_cast<float>(point.trajectory[i].second)});
  ...
  auto dec = [](auto v) { return v * 180.0 / std::numbers::pi_v<double>; };
  ...
  for (auto &point : tracked_.main()) {
    ...
    ImGui::Text("Target uid: %s\n"
                ...
                "Velocity (X, Y): \n\t\t(%0.3lf, %0.3lf)\n"
                "Acceleration (X, Y): \n\t\t(%0.3lf, %0.3lf)\n",
                point.description.c_str(), point.r, dec(point.phi), point.velocity.first, point.velocity.second,
                point.acceleration.first, point.acceleration.second);
    auto uid = point.uid;
    if (ImGui::Button(("Unlock##" + std::to_string(uid)).c_str()) && on_unlock_) { on_unlock_(uid); }
    ImGui::SameLine();
    if (ImGui::Button(("As target##" + std::to_string(uid)).c_str()) && on_target_) { on_target_(uid); }
    column_separator();
  }
  ImGui::Columns(1);
  ImGui::Separator();
...
void gui::radar_entry::process_mouse_click(const ImVec2 &center, double max_r, std::pair<double, double> const &scale) {
  if (ImGui::IsMouseClicked(ImGuiMouseButton_Right)) {
    ...
    v = v.in_polar();
    if (std::abs(cx) <= max_r && std::abs(cy) <= max_r && on_select_) { on_select_(v.r(), v.phi()); }
  }
}
